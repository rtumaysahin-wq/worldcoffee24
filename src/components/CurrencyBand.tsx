  ...
  return (
    <div className="flex items-center gap-3 p-3 md:p-4 bg-surface-container-lowest flex-1 min-w-0">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[10px] font-label uppercase tracking-widest text-secondary truncate">{country}</p>
        <p className="font-headline text-base md:text-lg font-bold text-primary">{item.label}</p>
      </div>
      <div className="text-right shrink-0">
        ...
        {hasChange && (
          <p className={`text-[11px] font-bold flex items-center justify-end gap-0.5 ${isUp ? "text-tertiary" : "text-error"}`}>
