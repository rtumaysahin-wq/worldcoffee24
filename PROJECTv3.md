...
- **Sohbet 7 (3-4 Nisan 2026):** Responsive/mobil optimizasyon (9 dosya: gap/padding/grid responsive). Döviz kuru bandı eklendi (CurrencyBand.tsx). Kontrast/okunabilirlik düzeltmeleri (chart butonları, kur çevirici, filtreler). Vercel Analytics incelendi (166 görüntüleme, 3 ülke). Google Search Console kurulumu + sitemap gönderimi + 5 sayfa indexleme isteği. Recharts Y-eksen font render sorunu düzeltildi (fontFamily açıkça tanımlandı). Fiyat değişimi yıllık→günlük olarak düzeltildi.
